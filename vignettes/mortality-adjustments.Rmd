...
## Applicaion to Partitioned Survival Models (PSM)
...
However as discussed in the accompanying manuscript, TTP - and therefore the PPD and PPS endpoints which directly follow - are often in practice implicitly defined in PSM economic model in order to calculate payoffs that depend on progression events. Two methods are possible:
* Simple - the hazard of TTP relative to PFS is proportionate to the progression events observed over the follow-up period relative to the total number of observed PFS events.
