...
# )
# drmd1_psm(ptdata=bosonc, dpam=params)
# # Add a lifetable constraint
# ltable <- tibble::tibble(lttime=0:20, lx=1-lttime*0.05)
# drmd1_psm(ptdata=bosonc, dpam=params, lifetable=ltable)
drmd1_psm <- function(ptdata, dpam, psmtype="simple", Ty=10, discrate=0, lifetable=NA, timestep=1) {
...
#   pps_cr = find_bestfit_spl(fits$pps_cr, "aic")$fit
# )
# drmd1_stm_cf(dpam=params)
# # Add a lifetable constraint
# ltable <- tibble::tibble(lttime=0:20, lx=1-lttime*0.05)
# drmd1_stm_cf(dpam=params, lifetable=ltable)
drmd1_stm_cf <- function(dpam, Ty=10, discrate=0, lifetable=NA, timestep=1) {
  # Declare local variables
...
#   pps_cr = find_bestfit_spl(fits$pps_cr, "aic")$fit
# )
# drmd1_stm_cr(dpam=params)
# # Add a lifetable constraint
# ltable <- tibble::tibble(lttime=0:20, lx=1-lttime*0.05)
# drmd1_stm_cr(dpam=params, lifetable=ltable)
drmd1_stm_cr <- function(dpam, Ty=10, discrate=0, lifetable=NA, timestep=1) {
  # Declare local variables
  Tw <- tvec <- sppd <- sttp <- sos <- NULL
  adjsppd <- adjos <- vn <- pf <- os <- NULL
  # Time horizon in weeks (ceiling)
